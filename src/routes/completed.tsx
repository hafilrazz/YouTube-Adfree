...
import { useCompleted, useVideosByIds } from "@/lib/user-data";
...
export const Route = createFileRoute("/completed")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Videos you've finished watching on Premium." },
      { property: "og:title", content: "Completed videos" },
      { property: "og:description", content: "Videos you've finished watching on Premium." },
...
function CompletedPage() {
  const { ids, clear } = useCompleted();
  const { data: videos = [] } = useVideosByIds(ids);
  return (
    <FakeTubeLayout>
      <div className="flex items-center gap-3 mb-6">
        <CheckCircle2 className="h-6 w-6 text-green-600" />
        <h1 className="text-2xl font-bold">Completed videos</h1>
      ...
      {videos.length === 0 ? (
        <div className="text-center py-20 border border-dashed rounded-2xl">
          <p className="font-semibold">Nothing completed yet</p>
          <p className="text-sm text-neutral-600 mt-1">Videos you finish watching show up here.</p>
